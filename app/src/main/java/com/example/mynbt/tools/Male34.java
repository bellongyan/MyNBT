...
public class Male34 {
    ...
    public Male34(int height_value, double weight_value, int vital_capacity_value, double fifty_race_value, double sit_and_reach_value, int jump_value, int pull_up_value, int one_thousand_value) {
...
//    计算 BMI 分数
    ...
    public void calcVitalCapacityScore() {
        ...
        if (getVital_capacity_value() < 2350) {
            vitalCapacityScore = 0.0;
        } else if (getVital_capacity_value() < 2520) {
            vitalCapacityScore = 10.0;
        } else if (getVital_capacity_value() < 2690) {
            vitalCapacityScore = 20.0;
        } else if (getVital_capacity_value() < 2860) {
            vitalCapacityScore = 30.0;
        } else if (getVital_capacity_value() < 3030) {
            vitalCapacityScore = 40.0;
        } else if (getVital_capacity_value() < 3200) {
            vitalCapacityScore = 50.0;
        } else if (getVital_capacity_value() < 3320) {
            vitalCapacityScore = 60.0;
        } else if (getVital_capacity_value() < 3440) {
            vitalCapacityScore = 62.0;
        } else if (getVital_capacity_value() < 3560) {
            vitalCapacityScore = 64.0;
        } else if (getVital_capacity_value() < 3680) {
            vitalCapacityScore = 66.0;
        } else if (getVital_capacity_value() < 3800) {
            vitalCapacityScore = 68.0;
        } else if (getVital_capacity_value() < 3920) {
            vitalCapacityScore = 70.0;
        } else if (getVital_capacity_value() < 4040) {
            vitalCapacityScore = 72.0;
        } else if (getVital_capacity_value() < 4160) {
            vitalCapacityScore = 74.0;
        } else if (getVital_capacity_value() < 4280) {
            vitalCapacityScore = 76.0;
        } else if (getVital_capacity_value() < 4400) {
            vitalCapacityScore = 78.0;
        } else if (getVital_capacity_value() < 4650) {
            vitalCapacityScore = 80.0;
        } else if (getVital_capacity_value() < 4900) {
            vitalCapacityScore = 85.0;
        } else if (getVital_capacity_value() < 5020) {
            vitalCapacityScore = 90.0;
        } else if (getVital_capacity_value() < 5140) {
    ...
    public void calcFiftyRaceScore() {
        ...
        if (getFifty_race_value() > 10.0) {
            ...
            fiftyRaceScore = 68.0;
        } else if (getFifty_race_value() > 7.8) {
            fiftyRaceScore = 70.0;
        } else if (getFifty_race_value() > 7.6) {
            fiftyRaceScore = 72.0;
        } else if (getFifty_race_value() > 7.4) {
            fiftyRaceScore = 74.0;
        } else if (getFifty_race_value() > 7.2) {
            fiftyRaceScore = 76.0;
            ...
            fiftyRaceScore = 90.0;
        } else if (getFifty_race_value() > 6.6) {
            fiftyRaceScore = 95.0;
        } else {
    ...
    public void calcSitReachScore() {
        ...
        if (getSit_and_reach_value() < -0.8) {
            sitReachScore = 0.0;
        } else if (getSit_and_reach_value() < 0.2) {
            sitReachScore = 10.0;
        } else if (getSit_and_reach_value() < 1.2) {
            sitReachScore = 20.0;
        } else if (getSit_and_reach_value() < 2.2) {
            sitReachScore = 30.0;
        } else if (getSit_and_reach_value() < 3.2) {
            sitReachScore = 40.0;
        } else if (getSit_and_reach_value() < 4.2) {
            sitReachScore = 50.0;
        } else if (getSit_and_reach_value() < 5.6) {
            sitReachScore = 60.0;
        } else if (getSit_and_reach_value() < 7.0) {
            sitReachScore = 62.0;
        } else if (getSit_and_reach_value() < 8.4) {
            sitReachScore = 64.0;
        } else if (getSit_and_reach_value() < 9.8) {
            sitReachScore = 66.0;
        } else if (getSit_and_reach_value() < 11.2) {
            sitReachScore = 68.0;
        } else if (getSit_and_reach_value() < 12.6) {
            sitReachScore = 70.0;
        } else if (getSit_and_reach_value() < 14.0) {
            sitReachScore = 72.0;
        } else if (getSit_and_reach_value() < 15.4) {
            sitReachScore = 74.0;
        } else if (getSit_and_reach_value() < 16.8) {
            sitReachScore = 76.0;
        } else if (getSit_and_reach_value() < 18.2) {
            sitReachScore = 78.0;
        } else if (getSit_and_reach_value() < 19.9) {
            sitReachScore = 80.0;
        } else if (getSit_and_reach_value() < 21.5) {
            sitReachScore = 85.0;
        } else if (getSit_and_reach_value() < 23.3) {
            sitReachScore = 90.0;
        } else if (getSit_and_reach_value() < 25.1) {
            sitReachScore = 95.0;
        } else {
            sitReachScore = 100.0;
        }
    ...
    public void calcJumpScore() {
        double jumpScore;
        if (getJump_value() < 185) {
            jumpScore = 0.0;
        } else if (getJump_value() < 190) {
            jumpScore = 10.0;
        } else if (getJump_value() < 195) {
            jumpScore = 20.0;
        } else if (getJump_value() < 200) {
            jumpScore = 30.0;
        } else if (getJump_value() < 205) {
            jumpScore = 40.0;
        } else if (getJump_value() < 210) {
            jumpScore = 50.0;
        } else if (getJump_value() < 214) {
            jumpScore = 60.0;
        } else if (getJump_value() < 218) {
            jumpScore = 62.0;
        } else if (getJump_value() < 222) {
            jumpScore = 64.0;
        } else if (getJump_value() < 226) {
            jumpScore = 66.0;
        } else if (getJump_value() < 230) {
            jumpScore = 68.0;
        } else if (getJump_value() < 234) {
            jumpScore = 70.0;
        } else if (getJump_value() < 238) {
            jumpScore = 72.0;
        } else if (getJump_value() < 242) {
            jumpScore = 74.0;
        } else if (getJump_value() < 246) {
            jumpScore = 76.0;
        } else if (getJump_value() < 250) {
            jumpScore = 78.0;
        } else if (getJump_value() < 258) {
            jumpScore = 80.0;
        } else if (getJump_value() < 265) {
            jumpScore = 85.0;
        } else if (getJump_value() < 270) {
            jumpScore = 90.0;
        } else if (getJump_value() < 275) {
            jumpScore = 95.0;
        } else {
            jumpScore = 100.0;
        }
    ...
    public void calcOneThousandScore() {
        ...
        if (getOne_thousand_value() > 370) {
            oneThousandScore = 0.0;
        } else if (getOne_thousand_value() > 350) {
            oneThousandScore = 10.0;
        } else if (getOne_thousand_value() > 330) {
            oneThousandScore = 20.0;
        } else if (getOne_thousand_value() > 310) {
            oneThousandScore = 30.0;
        } else if (getOne_thousand_value() > 290) {
            oneThousandScore = 40.0;
        } else if (getOne_thousand_value() > 270) {
            oneThousandScore = 50.0;
        } else if (getOne_thousand_value() > 265) {
            oneThousandScore = 60.0;
        } else if (getOne_thousand_value() > 260) {
            oneThousandScore = 62.0;
        } else if (getOne_thousand_value() > 255) {
            oneThousandScore = 64.0;
        } else if (getOne_thousand_value() > 250) {
            oneThousandScore = 66.0;
        } else if (getOne_thousand_value() > 245) {
            oneThousandScore = 68.0;
        } else if (getOne_thousand_value() > 240) {
            oneThousandScore = 70.0;
        } else if (getOne_thousand_value() > 235) {
            oneThousandScore = 72.0;
        } else if (getOne_thousand_value() > 230) {
            oneThousandScore = 74.0;
        } else if (getOne_thousand_value() > 225) {
            oneThousandScore = 76.0;
        } else if (getOne_thousand_value() > 220) {
            oneThousandScore = 78.0;
        } else if (getOne_thousand_value() > 212) {
            oneThousandScore = 80.0;
        } else if (getOne_thousand_value() > 205) {
            oneThousandScore = 85.0;
        } else if (getOne_thousand_value() > 200) {
            oneThousandScore = 90.0;
        } else if (getOne_thousand_value() > 195) {
